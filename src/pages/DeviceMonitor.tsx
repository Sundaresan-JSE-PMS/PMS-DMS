...
export const DeviceMonitor = (currentRoom: any) => {
    ...
    const [isLoadingCoolingMachine, setIsLoadingCoolingMachine] =
        useState(true);
    ...
    interface SelectedCardData {
        ...
        patient: any; // Replace 'any' with a more specific type if available
        observation_resource: any; // Replace 'any' with a more specific type if available
        communication_resource: any; // Replace 'any' with a more specific type if available
        newData: any; // Replace 'any' with the actual type of newData
    }
    ...
    const handleCardClick = (cardData: SelectedCardData) => {
        ...
        if (selectedCard && selectedCard.device_id === cardData.device_id) {
            setSelectedCard(null); // Deselect the card
        } else {
            setSelectedCard(cardData); // Select the new card
        }
    };

    const handleCloseDetails = () => {
        setSelectedCard(null); // Resets the selected card to hide details
    };
        ...
        socket.onmessage = (data) => {
            ...
            if (recieved_data.location.split("/")[0] == "Observation") {
                fetch(` https://pmsind.co.in:5000/${recieved_data.location}`, {
                    ...
                    .then((data) => {
                        let temp = String(
                            data.device?.reference?.split("/")[1]
                        );

                        console.log(data);
                        setParentObs((prevparentobs) => ({
                            ...prevparentobs,
                            [temp]: data,
                        }));
                ...
                recieved_data.location.split("/")[0] == "Communication"
            ) {
                fetch(
                    ` https://pmsind.co.in:5000/${
                        JSON.parse(data.data).location
                    }`,
                    {
                        credentials: "omit",
                        headers: {
    ...
    useEffect(() => {
        if (currentRoom != "") {
            ...
            // console.log(url)
            fetch(
                ` https://pmsind.co.in:5000/Device?location=${currentRoom.currentRoom}`,
                {
                    credentials: "omit",
                    headers: {
                        Authorization:
    ...
    useEffect(() => {
        devices.entry?.map((device) => {
            ...
            if (device.resource.patient) {
                fetch(
                    ` https://pmsind.co.in:5000/Patient/${
                        device.resource.patient.reference.split("/")[1]
                    }`,
                    {
                        credentials: "omit",
                        headers: {
                ...
                fetch(
                    ` https://pmsind.co.in:5000/Observation?patient=${
                        device.resource.patient.reference.split("/")[1]
                    ...
                    .then((data) => {
                        if (!data.entry) {
                            console.log(
                                ` https://pmsind.co.in:5000/Observation?patient=${
                                    device.resource.patient.reference.split(
                                        "/"
                                    )[1]
                                }&_count=1&_sort=-_lastUpdated`
                            );
                            ...
                            setParentObs((prevParentobs) => ({
                                ...prevParentobs,
                                [temp]: data.entry[0]["resource"],
                            }));
                        }
                    });

                fetch(
                    ` https://pmsind.co.in:5000/Communication?sender=${device.resource.id}&_count=1&_sort=-_lastUpdated`,
                    {
                        credentials: "omit",
                        headers: {
                            Authorization:
    ...
    const warmer = devices.entry?.map((device) => {
        if (
            String(device.resource.identifier[1]?.value) ==
            ...
            //     }
            // }
            const cardProps = correct
                ? {
                      device_id: String(device.resource.identifier[0].value),
                      device_resource_id: String(device.resource.id),
                      patient: patient[String(device.resource.id)], // Adjusted
                      observation_resource:
                          parentobs[String(device.resource.id)],
                      communication_resource:
                          parentcomm[String(device.resource.id)],
    ...
    const incubator = devices.entry?.map((device) => {
        if (
            String(device.resource.identifier[1]?.value) ==
            "Intensive Neonatal Care Center"
        ) {
            var correct = false;
            // var temp = String(device.resource.id)
            if (
    ...
    const syringe = devices.entry?.map((device) => {
        if (String(device.resource.identifier[1]?.value) === "PMS-SYRINGE") {
            var correct = false;
            // var temp = String(device.resource.id)
            if (
                device.resource.patient &&
        ...
        >
            <Box
                sx={{
                    ...
                    gap: 2, // Adjust gap as needed between the two main Boxes
                }}
            >
                {/* Left Side: Accordions or content */}
                <Box
                    sx={{
                        width: selectedCard ? "50%" : "100%", // Occupy full width if no card selected, else 50%
                        display: "flex",
                        flexDirection: "column",
                        gap: 2, // Adjust gap between accordions as needed
                        height: "100vh", // Full screen height
                        overflow: "auto", // Enables scrolling for overflow content
                        maxWidth: "100%", // Ensures it does not exceed the viewport width
                        maxHeight: "89vh", // Ensures it does not exceed the viewport height, important for mobile
                        ...
                        "&::-webkit-scrollbar-thumb": {
                            background: "#124D81", // Blue scrollbar thumb
                            borderRadius: "1px", // Optional: Adds roundness to the scrollbar thumb
                        },
                        // For Firefox
                        scrollbarWidth: "thin",
                        scrollbarColor: "#124D81 transparent", // Blue thumb on transparent track
                    }}
                >
                    {isAuthenticated ? (
                        <>
 ...
 <DummyCard/> */}
                ...
                {/* Right Side: NewDeviceDetails (Shown only if a card is selected) */}
                {selectedCard && (
                    <Box
                        sx={{
                            width: "50%", // Always takes up 50% when displayed
                            display: "flex",
                            justifyContent: "center",
                            alignItems: "center",
                            padding: 2, // Add padding if needed for visual spacing inside the details box
                            height: "100vh", // Full screen height
                            overflow: "auto", // Enables scrolling for overflow content
                            maxWidth: "100%", // Ensures it does not exceed the viewport width
                            maxHeight: "89vh", // Ensures it does not exceed the viewport height, important for mobile
