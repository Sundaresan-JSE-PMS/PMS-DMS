...
export const ServicePage: React.FC <ServicePageProps> = ({ darkTheme }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        try {
          const response = await fetch('https://pmsserver.local/fhir/Organization/', {
  ...
  const handleAddOrganization = () => {
    ...
    fetch('https://pmsserver.local/fhir/Organization/', {
      credentials: "omit",
      method: "POST",
    ...
    .then((response) => {
      ...
      return fetch('https://pmsserver.local/fhir/Organization/', {
        headers: {
          Authorization: 'Basic ' + btoa('fhiruser:change-password'),
        },
      })
      ...
      {isAuthenticated && (
        <div>
          <Stack width={'100%'} direction={'row'} justifyContent={'center'} textAlign={'center'}>
            <Typography variant="h5" color={ darkTheme ? 'white' : '#124D81'}>All Hospitals</Typography>
          </Stack>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            ...
            >
              {organizations.map((org) => (
                <ServiceCard key={org.resource.id} organizationData={org.resource} organizationId={''} OrganizationName={''} deviceChange={function (): void {
