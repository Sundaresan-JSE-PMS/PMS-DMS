...
export const CICCard: FC<DeviceDetails> = (props): JSX.Element => {
    ...
    const onClickHandler = () => {
        ...
        // Assuming `props.onClick` is the function passed down from the parent
        props.onClick({
            ...props,
            newData, // Include newData in the object passed to onClick.
          });
    };
    ...
    useEffect(() => {
        ...
        if (
            props.observation_resource?.component?.[1] &&
            runNo >= 2 &&
            props.communication_resource?.extension?.[1]
        ) {
            setNewData(true);
            ...
            for (
                ...
                console.log(
                    props.communication_resource?.extension[1]
                        ?.valueCodeableConcept?.coding[i]?.code
                );
                if (
                    props.communication_resource?.extension[1]
                        ?.valueCodeableConcept?.coding[i]?.code ==
                    ...
                    setAlarmColor("red");
                    setAlarm(
                        props.communication_resource.extension[0]
                            .valueCodeableConcept.coding[i].display
                    );
                    break;
                } else {
    ...
    function findData(x: string) {
        let index = props?.observation_resource?.component.findIndex(
            (item) => item.code.text === x
        );
        if (index == -1) {
            return { data: "--", unit: "--" };
        }
        let data = Number(
            props.observation_resource.component[index].valueQuantity.value
        );
            ...
            <ButtonBase sx={{ width: "100%", borderRadius: "25px" }}>
                ...
                <Card
                    style={{
                        ...
                        borderRadius: "25px",
                        height: "300px",
                        opacity: newData ? 1 : 0.7,
                        border: `5px solid ${
                            isBlinking ? alarmColor : "#E4E4E4"
                        }`,
                    }}
                >
                    {newData ? (
