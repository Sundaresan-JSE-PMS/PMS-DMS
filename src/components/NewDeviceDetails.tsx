...
export const NewDeviceDetails: FC<DeviceDetails> = (props): JSX.Element => {
    ...
    const infscrollfunc = (page: Number) => {
        fetch(`https://pmsserver.local/fhir/Communication/${props.communication_resource.id}/_history/?_page=${page}`, {
  ...
  if (item.text !== '') {
    ...
    useEffect(() => {
        if(props.communication_resource?.id!=null){
            ...
            fetch(`https://pmsserver.local/fhir/Communication/${props.communication_resource.id}/_history/`, {
            credentials: "omit",
            headers: {
    ...
    function getDataForGraph(page: number, when: string) {
        ...
        function fetchData(when: string, times:number): Promise<void> {
            return fetch(`https://pmsserver.local/fhir/Observation/${props.observation_resource.id}/_history?_count=1&_since=${when}&_page=${page}`,{
                credentials: "omit",
                method: "GET",
                headers: {
                    Authorization: "Basic "+ btoa("fhiruser:change-password"),
            ...
            .then((data: any) => {
                ...
                if(data.total>0){
                    ...
                    return fetch(`https://pmsserver.local/fhir/Observation/${props.observation_resource.id}/_history?_count=1&_since=${when}&_page=${lastpage}`,{
                        credentials: "omit",
                        method: "GET",
                        headers: {
                            Authorization: "Basic "+ btoa("fhiruser:change-password"),
    ...
    useEffect(() => {
        ...
        if(timeFrame!=-1){
            if(timeFrame==0){
                let prevdate = ""
                url.push(`https://pmsserver.local/fhir/Observation/${props.observation_resource?.id}/_history?_since=${currentDate}T00:00:00Z&_count=10000`)
                Promise.all(
                    url.map((query) => {
                        return fetch(query, {
                            credentials: "omit",
