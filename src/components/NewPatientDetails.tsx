...
export const NewPatientDetails: FC<PatientDetails> = (props): JSX.Element => {
    ...
    function getDataForGraph(index: number,page: number, when: string) {
        ...
        function fetchData(when: string, times:number): Promise<void> {
            return fetch(`https://pmsserver.local/fhir/Observation/${props.observation_resource[index]?.id}/_history?_count=1&_since=${when}&_page=${page}`,{
                credentials: "omit",
                method: "GET",
                headers: {
            ...
            .then((data: any) => {
                ...
                if(data.total>0){
                    ...
                    return fetch(`https://pmsserver.local/fhir/Observation/${props.observation_resource[index].id}/_history?_count=1&_since=${when}&_page=${lastpage}`,{
                        credentials: "omit",
                        method: "GET",
                        headers: {
                            Authorization: "Basic "+ btoa("fhiruser:change-password"),
        ...
        if(timeFrame!=-1){
            if(timeFrame==0){
                ...
                props.observation_resource?.map((val,i) => {
                    console.log(val)
                    let prevdate = ""
                    url.push(`https://pmsserver.local/fhir/Observation/${props.observation_resource[i].id}/_history?_since=${currentDate}T00:00:00Z&_count=10000`)
                    Promise.all(
    ...
    const infscrollfunc = (page: Number) => {
        props.communication_resource.map((communication,index ) => {
            fetch(`https://pmsserver.local/fhir/Communication/${communication.id}/_history/?_page=${page}`, {
